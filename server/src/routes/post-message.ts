...
const getMessage = (message, persona) => {
  ...
  if (Number.isInteger(message)) {
    ...
    switch (persona) {
      ...
      case "recruiter":
      default:
        return `${companyContext}. textReponse: You are direct messaging a co-worker from the company that has come across Nandyala's resume, you WILL intiate by greeting the person and by expressing excitement for the canidate  while keeping within the character limit. ${optionsMessage}`
    }
  }
